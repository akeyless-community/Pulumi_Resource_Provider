...
# mise (https://mise.jdx.dev) pins golangci-lint from mise.toml. Builds work without it; install mise
# if you want `make lint_provider` to use the pinned linter version.
...
mise_env:
	@if [ -n "$(MISE)" ]; then $(MISE) env -q >/dev/null; fi

.make/mise_install:
	@if [ -n "$(MISE)" ]; then $(MISE) install -q; else echo "Note: mise not installed; skipping (optional). Install: brew install mise  or  https://mise.jdx.dev"; fi
	@touch $@
...
# `cmd/pulumi-resource-akeyless/schema.json` is valid and up to date.
